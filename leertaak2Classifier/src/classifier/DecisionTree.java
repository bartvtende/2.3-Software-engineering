...
public class DecisionTree implements Classifier {
	...
	/**
	 * Assign a category to an item. This is the implementation of algorithm 4.1
	 * on page 53 of the book.
	 * 
	 * @param item
	 *            The Item which has to be classified by the algorithm.
	 * @return A String describing the name of category to which the item
	 *         belongs.
	 */
	...
	/**
	 * Create a new DecisionTree based upon a given trainingsset and
	 * featurelist.
	 * 
	 * @param trainingsSet
	 * @param features
	 */
	public DecisionTree(Map<Item, String> trainingsSet,
			Map<String, FeatureType> features) {
		root = buildDecisionTree(trainingsSet, features);
	}

	/**
	 * This method implements algorithm 4.2 on page 54 of the book. Input : a
	 * training set of items T, a set of features F Output: a decision tree
	 * which can be used to classify other items.
	 * 
	 * @param trainingsSet
	 * @param features
	 * @return The root Node of the decisiontree created.
	 */
	private static Node buildDecisionTree(Map<Item, String> trainingsSet,
			Map<String, FeatureType> features) {
		// 1) 2ab) if the attribute set is empty or the items belong to a single
		// class
		...
		// (in the car example the possible values of the airco feature are
		// yes/no.
		...
		Map<String, HashMap<Item, String>> partitions = performSplit(
				trainingsSet, splitFeature, splitType.allowedValues());
		for (Iterator<String> iter = partitions.keySet().iterator(); iter
				.hasNext();) {
			String value = iter.next();
			// a) be ni the result of a recursive execution of this algorithm
			// where
			...
			Map<String, FeatureType> remainingFeatures = new HashMap<String, FeatureType>(
					features);
	...
	/**
	 * Determines the category of a set of items. Returns "?" when the items
	 * have
	 */
	...
	/**
	 * Determines the "best" split attribute (feature) for a set of items, it is
	 * based on the information gain criterion. Thus to determine which feature
	 * is "best" (which feature contains the most information)the formulas on
	 * page 54 and 55 are used.
	 */
	private static String selectSplit(Map<Item, String> trainingsSet,
			Map<String, FeatureType> features) {
		...
		while (attr.hasNext()) {
			...
			double gain = evaluateSplitGain(trainingsSet, candidate,
					type.allowedValues());
			if (gain > maxGain) {
				maxGain = gain;
				split = candidate;
	...
	/**
	 * Executes the split of a trainingsset according to a split feature. It
	 * returns the resulting sets of items, each of them being labeled with the
	 * relative feature value. (in our example: feature=airco, the result is
	 * {all cars with airco=yes}, {all cars with airco=no}) By the way, these
	 * generics look great, don't they? ;-)
	 */
	private static Map<String, HashMap<Item, String>> performSplit(
			Map<Item, String> trainingsSet, String split,
	...
	/**
	 * Evaluates the information gain that would derive from splitting a set of
	 * items according to a given split attribute.
	 */
	private static double evaluateSplitGain(Map<Item, String> items,
			String split, Collection<String> possibleValues) {
		double origInfo = information(items);
		double splitInfo = 0;
		Map<String, HashMap<Item, String>> partitions = performSplit(items,
				split, possibleValues);
	...
	/**
	 * Computes the information content of a set of items. The classes represent
