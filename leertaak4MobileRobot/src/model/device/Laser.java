...
public class Laser extends ScanDevice {
	...
	public Laser(String name, MobileRobot robot, Position localPos,
	...
	public double read(boolean first, boolean opaque) {
		...
		Point2D front = new Point2D.Double(localPosition.getX() + RANGE
				* Math.cos(localPosition.getT()), localPosition.getY() + RANGE
				* Math.sin(localPosition.getT()));
		...
		for (int i = 0; i < environment.getObstacles().size(); i++) {
			...
			if (!obstacle.getOpaque()) {
				double dist = pointToObstacle(obstacle.getPolygon(), centre,
						front, first);
